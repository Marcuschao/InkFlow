...
public class SearchReindexService {
    ...
    public SearchReindexResult reindexAllPublished() {
        if (!meilisearchClient.isAvailable()) {
            throw new IllegalStateException("Elasticsearch unavailable at " + searchProperties.getHost());
        }
        return distributedLockService.executeWithLock("lock:search:reindex", this::doSwapReindex);
    }

    public SearchReindexResult reindexAllPublishedOnStartup() {
        if (!meilisearchClient.isAvailable()) {
            throw new IllegalStateException("Elasticsearch unavailable at " + searchProperties.getHost());
        }
        return doSwapReindex();
    }

    private SearchReindexResult doSwapReindex() {
        String primaryIndex = searchProperties.getIndex();

        meilisearchClient.recreateIndex(primaryIndex);
        ...
        while (true) {
            ...
            searchIndexService.reindexBatch(primaryIndex, page.getRecords());
        ...
        return new SearchReindexResult(total, primaryIndex, true);
    }
}
