...
public class MeilisearchSearchService {
    ...
    public MeilisearchSearchService(MeilisearchClient meilisearchClient) {
        this.meilisearchClient = meilisearchClient;
    }

    public boolean canSearch() {
        return meilisearchClient.isAvailable();
    }
    ...
    public PageResult<ArticleSearchHitVo> search(SearchPageQuery query) {
        ...
        JsonNode root = meilisearchClient.search(kw, size, offset);
        JsonNode hits = root.path("hits");
        long total = root.path("estimatedTotalHits").asLong(root.path("totalHits").asLong(hits.size()));
    ...
    public List<ArticleSearchSuggestVo> suggest(String keyword, int limit) {
        ...
        JsonNode root = meilisearchClient.search(keyword.trim(), limit, 0);
        ...
        for (JsonNode hit : root.path("hits")) {
            ...
            String ht = formatted.path("title").asText("");
            vo.setHighlightTitle(StringUtils.hasText(ht) ? ht : HtmlUtils.htmlEscape(vo.getTitle()));
            list.add(vo);
        }
        return list;
    }
    ...
    private ArticleSearchHitVo toHitVo(JsonNode hit) {
        ...
        JsonNode formatted = hit.path("_formatted");
        String ht = formatted.path("title").asText("");
        String hs = formatted.path("summary").asText("");
        vo.setHighlightTitle(StringUtils.hasText(ht) ? ht : HtmlUtils.htmlEscape(vo.getTitle()));
        vo.setHighlightSummary(StringUtils.hasText(hs) ? hs : HtmlUtils.htmlEscape(
                StringUtils.hasText(vo.getSummary()) ? vo.getSummary() : ""));
        return vo;
    }
