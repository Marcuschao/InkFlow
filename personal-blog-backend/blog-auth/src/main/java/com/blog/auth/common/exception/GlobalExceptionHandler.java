package com.blog.auth.common.exception;
...
public class GlobalExceptionHandler {
    ...
    public ResponseEntity<Result<?>> handleIllegalArgument(IllegalArgumentException e) {
        ...
        return ResponseEntity.status(HttpStatus.BAD_REQUEST).body(Result.fail(400, e.getMessage(), null));
    }

    @ExceptionHandler(MaxUploadSizeExceededException.class)
    public ResponseEntity<Result<?>> handleMaxUpload(MaxUploadSizeExceededException e) {
        return ResponseEntity.status(HttpStatus.BAD_REQUEST).body(Result.fail(400, "文件不能超过 10MB", null));
    }

    @ExceptionHandler(MultipartException.class)
    public ResponseEntity<Result<?>> handleMultipart(MultipartException e) {
        log.warn("上传解析失败: {}", e.getMessage());
        return ResponseEntity.status(HttpStatus.BAD_REQUEST).body(Result.fail(400, "文件上传失败，请重试", null));
    }
