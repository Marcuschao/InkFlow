  ...
  <div class="admin-page">
    <div class="container">
      <header class="dash-header ds-admin-header" style="margin-bottom: 24px;">
        <div>
          <h1 class="ds-page-title">保鲜报告</h1>
          <p class="ds-page-sub">AI 保鲜草稿 MinIO 持久化记录</p>
        </div>
        <n-space class="dash-actions" :size="12">
          <router-link to="/admin/freshness"><n-button>内容保鲜</n-button></router-link>
      ...
      <n-card :bordered="true" style="margin-bottom: 16px;">
        <n-space align="center" :size="12">
          <span>文章 ID</span>
          <n-input v-model:value="articleIdRaw" style="width: 120px;" placeholder="可选" clearable />
          <n-button @click="reload">筛选</n-button>
        </n-space>
      </n-card>
...
import { NAlert, NButton, NCard, NDataTable, NEmpty, NInput, NModal, NSpace } from 'naive-ui';
import Pagination from '../../components/Pagination.vue';
import MarkdownRenderer from '../../components/MarkdownRenderer.vue';
import { fetchFreshnessReports, fetchReportContent } from '../../api/reports';
...
async function openReport(row) {
  try {
    modalTitle.value = row.title || '保鲜报告';
...
const columns = [
  ...
  { title: '文章', key: 'targetId', width: 72 },
...
async function load() {
  ...
  loadErr.value = '';
  try {
    const aid = articleIdRaw.value?.trim();
    const articleId = aid ? Number(aid) : undefined;
    const data = await fetchFreshnessReports(articleId, page.value, pageSize.value);
